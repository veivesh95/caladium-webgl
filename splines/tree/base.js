...
function webGLStart() {
  ...
  prMatrix.perspective(70, c_w / c_h, 0.01, 100);
  ...
  rand();
  pt = new Float32Array(2700000);
  ind = new Uint32Array(1400000);
...
function tree() {
  ...
  var rg = height[age - 1] * 0.1; //.1+.05*age
  // var rg = age * 0.1; //.1+.05*age
  // base(b1, 0, 1.5*rg,1.2*rg,.3*rg, .2,1, 1) // to remove the trunk base
  twig(b1, 0.1, 0, 1.2 * rg, rg, 1, 1, 2, 1);
...
//   if (first) twig(b, th1 * 0.7, fi1, 0.8 * sr, sr1, sb, sb, stu, di);
//   else twig(b, th1, fi1, 0.8 * sr, sr1, sb, sb, stu, di);
...
//   if (first) {
//     first = false;
//     branch(it, b, sr1, sb * sclen * 0.6, stu, div, age2);
...
function drawScene() {
  ...
  gl.uniform1f(uAnim, 0.1 * Math.sin(anim) + 0.05 * Math.sin(2.7 * anim));
  gl.uniform1i(tex, 0);
...
async function start() {
  ...
  //  console.log('heightCount', heightLength);
  webGLStart();
  setTimeout(() => {
    setupGui();
  }, 1500);
...
function setupGui() {
  ...
  gui.add(tFork, "animation").onChange(function (v) {
    bAnim = v;
    animate();
  });

  gui
    .add(tFork, "age", 1, height.length)
    .step(1)
    .onChange(function (v) {
      age = v;
      tree();
  ...
  gui.add(tFork, "green").onChange(function (v) {
    green = v;
    tree();
  });

  gui.add(tFork, "leaves").onChange(function (v) {
    leaf = v;
    tree();
  });

  gui.add(tFork, "length", 0.5, 1).onChange(function (v) {
    sclen = v;
    tree();
  });

  var fFork = gui.addFolder("Fork");
  fFork.add(tFork, "bend1", 0.1, 1.5).onChange(function (v) {
    thr1 = v;
    tree();
  });

  fFork.add(tFork, "bend2", 0.1, 1.5).onChange(function (v) {
    thr2 = v;
    tree();
  });
  ...
  fTwig1.add(tTwig1, "bend", -1.5, 1.5).onChange(function (v) {
    th1 = v;
    tree();
  });
  ...
  fTwig1.add(tTwig1, "scale_r1", 0.4, 0.8).onChange(function (v) {
    scr1 = v;
    tree();
  });
  ...
  fTwig2.add(tTwig2, "bend", -1.5, 1.5).onChange(function (v) {
    th2 = v;
    tree();
  });
